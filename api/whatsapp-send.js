...
// Send a WhatsApp text message via Green API. Manager/admin only.
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(
      `https://api.green-api.com/waInstance${ID}/sendMessage/${GREEN_TOKEN}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chatId: waChatId, message: text }),
      }
    )
